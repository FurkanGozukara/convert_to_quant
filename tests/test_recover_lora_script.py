...
class TestRecoverLoraScript(unittest.TestCase):
    def setUp(self):
        ...
        save_file(
            {self.layer_name: qdata, f"double_blocks.0.img_attn.qkv.weight_scale": block_scales}, self.quant_mxfp8_path
        )
    ...
    def test_skip_logic(self):
        ...
        if block_scales.dtype == torch.float8_e8m0fnu:
            block_scales = block_scales.view(torch.uint8)
        save_file({deep_layer: qdata, f"double_blocks.5.img_mlp.0.weight_scale": block_scales}, self.quant_mxfp8_path)
